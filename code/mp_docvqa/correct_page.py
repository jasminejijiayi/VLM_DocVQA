...
def preprocess_image(example):
    ...
    '''
    # Get the answer page index (0-based index)
    ...
    if image_key in example and example[image_key] is not None:
        ...
        # Fallback to image_1 if the correct image is not available
        image = example["image_1"]
        print(f"Correct image {image_idx} not available, falling back to image_1")
    
    # TODO: More preprocessing methods could be added here
    # For example: image enhancement, OCR preprocessing, etc.
...
def main(args):
    ...
    if args.use_wandb:
        run_name = f"methodx_eval_{datetime.now().strftime('%Y%m%d_%H%M')}" if args.eval_only else f"methodx_{datetime.now().strftime('%Y%m%d_%H%M')}"
        run = wandb.init(
            ...
            config={
                ...
                "experiment_type": "methodx",
                "image_selection": "correct_page",
    ...
    if not args.eval_only:
        ...
        for i, example in enumerate(tqdm(ds, desc="Generating answers", total=len(ds), leave=True, position=0)):
            ...
            used_page_idx = example["answer_page_idx"]  # Since baseline uses the first image
